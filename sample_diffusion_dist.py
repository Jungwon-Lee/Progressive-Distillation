...
def run(args):
    ...
    # n_samples = opt.n_samples // opt.n_gpu # n_sample per 1 gpu
...
def parse(parser):
    ...
    # write config out
    sampling_file = os.path.join(opt.logdir, "sampling_config.yaml")
...
if __name__ == "__main__":
    ...
    # model.model.diffusion_model.make_block_buffer()

    # Q.set_n_lv(model, opt.n_bit_w, opt.n_bit_a, opt.use_dynamic)
    # run
    run([0, model, opt.gpus, opt])

    # with mp.Pool(opt.n_gpu) as p:
    #     args = []
    #     for idx, gpu in enumerate(gpus):
    #         args.append([idx, model, gpu, opt])
